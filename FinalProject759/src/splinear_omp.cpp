...
#define matmul_omp matmul
#define inner_product_omp inner_product
#define norm_omp norm

double inner_product_omp(const double *x1, const double *x2, unsigned int n,
                         const size_t threads_num);

void LUdecomposition_omp(double **a, double **l, double **u, int n,
                         const size_t threads_num);

void matmul_omp(double **A, const double *x, unsigned int n, double *ret,
                const size_t threads_num);
...
void matmul_omp(double **A, const double *x, unsigned int n, double *ret,
  ...
  for (unsigned int i = 0; i < n; i++) {
    sum = 0;
#pragma omp parallel for simd reduction(+ : sum) num_threads(threads_num)
...
void splinear_solver_omp(double **A, const double *b, const unsigned int n,
  ...
  // LU decomposition
  LUdecomposition_omp(A, L, U, n, threads_num);

  // omp_set_num_threads(threads_num);
  double sum;
  for (unsigned i = 0; i < n; i++) { // forward subtitution method
    sum = 0;
#pragma omp parallel for reduction(+ : sum) num_threads(threads_num)
...
void splinear_solver2_omp(double **A, const double *b, const unsigned int n,
  ...
  for (int t = 0; t < max_iter; t++) {
    matmul(A, p0, n, w, threads_num);
    alpha = inner_product(r0, r0, n, threads_num) /
            inner_product(p0, w, n, threads_num);
    ...
    if (norm(r, n, threads_num) < tol)
      return;
    else {
      beta = inner_product(r, r, n, threads_num) /
             inner_product(r0, r0, n, threads_num);
